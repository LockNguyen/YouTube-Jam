import type { Handler } from '@netlify/functions'
import { performJumpToSong } from '../shared/queueService'
import { validateHostKey, parseBody, requireString } from '../shared/validation'
import { success, error, headers } from '../shared/responses'

const handler: Handler = async (event) => {
  if (event.httpMethod === 'OPTIONS') {
    return { statusCode: 204, headers }
  }

  if (event.httpMethod !== 'POST') {
    return { ...error('Method not allowed', 405), headers }
  }

  if (!validateHostKey(event.headers as Record<string, string | undefined>)) {
    return { ...error('Unauthorized', 401), headers }
  }

  const body = parseBody(event.body)
  const roomId = requireString(body, 'roomId')
  const songId = requireString(body, 'songId')

  if (!roomId || !songId) {
    return { ...error('roomId and songId are required'), headers }
  }

  try {
    await performJumpToSong(roomId, songId)
    return { ...success({ songId }), headers }
  } catch (err) {
    console.error('[jumpToSong] Error:', err)
    return { ...error('Failed to jump to song', 500), headers }
  }
}

export { handler }
